...
(function (global) {
  ...
  function mountMarkers(root, onOpen) {
    (root || document).querySelectorAll('[data-avatar-portal][data-design-key="brand-name"]').forEach((marker) => {
  ...
  function markEditablePage(root) {
    ...
    scope.querySelectorAll(".avatar-wave-marker, .card-avatar-marker").forEach((marker) => marker.remove());
  }
